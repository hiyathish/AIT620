...
x_vals = cluster_centers_2d[:, 0]
y_vals = cluster_centers_2d[:, 1]

plt.xlim(x_vals.min() - offset * np.ptp(x_vals),
         x_vals.max() + offset * np.ptp(x_vals))

plt.ylim(y_vals.min() - offset * np.ptp(y_vals),
         y_vals.max() + offset * np.ptp(y_vals))
