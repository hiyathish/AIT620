...
df = yf.download("INTC", start=start, end=end)

# Convert to old-style tuple format
stock_quotes = [
    (idx.to_pydatetime(), row.Open, row.High, row.Low, row.Close, row.Volume)
    for idx, row in df.iterrows()
]

print(stock_quotes[:5])

# Extract the closing quotes everyday
closing_quotes = np.array([quote[4] for quote in stock_quotes])
